...
import { PlayedBadge, DailyTripleBanner } from "@/components/DailyProgress";
  ...
  return (
    <Link
      ...
      className={`group relative block overflow-hidden rounded-2xl border transition-all duration-300 ${
        isLive
          ? "border-zinc-300 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 hover:border-amber-500/60 dark:hover:border-amber-500/30 hover:bg-amber-50/30 dark:hover:bg-zinc-900/70"
          : "border-zinc-300/50 dark:border-zinc-800/30 bg-zinc-50 dark:bg-zinc-900/25 opacity-60"
      }`}
      ...
      <div className="relative p-6">
        <div className="flex items-center gap-2 mb-3">
          <span className={`text-[10px] font-semibold uppercase tracking-[0.2em] ${
            isLive ? "text-amber-600 dark:text-amber-500/70" : isPreview ? "text-zinc-500 dark:text-zinc-600" : "text-zinc-400 dark:text-zinc-700"
          }`}>
            {isLive ? "Play Now" : isPreview ? "Preview" : "Coming Soon"}
          </span>
          {isLive && <PlayedBadge game={game.slug} />}
        </div>
        <h2 className="font-display text-2xl md:text-3xl font-bold text-zinc-900 dark:text-zinc-100 leading-none flex items-center gap-2">
        ...
        {isLive && (
          <div className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-amber-600 dark:text-amber-400 group-hover:text-amber-700 dark:group-hover:text-amber-300 transition-colors">
            Play
      ...
      <div className="mx-auto max-w-4xl px-5 md:px-8 py-12 md:py-20">
        ...
        <div className="text-center mb-12 md:mb-16 animate-fadeIn">
          ...
          <h1 className="font-display text-4xl md:text-6xl font-extrabold tracking-tight mb-2">
            <span className="text-amber-600 dark:text-amber-400">Movie</span><span className="text-zinc-900 dark:text-zinc-300">Night</span>
          </h1>
          <p className="text-sm md:text-base text-zinc-950 dark:text-zinc-500 max-w-md mx-auto">
            Play daily movie puzzle games and fast-paced movie trivia challenges about the films you love.
          </p>
        </div>

        {/* Daily Triple Progress */}
        ...
        <div className="mb-6 animate-fadeIn" style={{ animationDelay: "0.1s" }}>
          <Link
            ...
            className="group relative block overflow-hidden rounded-2xl border border-zinc-300 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 hover:border-amber-500/70 dark:hover:border-amber-500/40 transition-all duration-300"
          >
            ...
            <div className="relative p-8 md:p-10 md:flex md:items-start md:justify-between md:gap-8">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-amber-600 dark:text-amber-500/70">Featured</span>
                  <PlayedBadge game="roles" />
                </div>
                <h2 className="mt-2 font-display text-3xl md:text-5xl font-extrabold text-zinc-900 dark:text-zinc-100 leading-none flex items-center gap-3">
                  <span>{"\u{1F3AD}"}</span> ROLES
                </h2>
                <p className="mt-3 text-sm md:text-base text-zinc-600 dark:text-zinc-400 max-w-lg leading-relaxed">
                ...
                <div className="mt-6 inline-flex items-center gap-2 rounded-xl bg-amber-500 px-5 py-3 text-sm font-bold uppercase tracking-wider text-zinc-950 dark:text-zinc-950 shadow-lg shadow-amber-500/20 group-hover:bg-amber-400 transition-all">
                  Play Now
                  <svg className="w-4 h-4 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
                  </svg>
              ...
              <div className="hidden md:flex flex-col items-end gap-1.5 shrink-0 pt-1">
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-1.5 text-center">
                  <p className="text-lg font-bold text-amber-500 dark:text-amber-400 leading-none">#{puzzleNumber}</p>
                  <p className="text-[8px] uppercase tracking-widest text-amber-600/50 dark:text-amber-500/50 mt-0.5">Daily Puzzle</p>
                </div>
                {gamesPlayed !== null && gamesPlayed > 0 && (
                  <p className="text-[10px] text-zinc-500 dark:text-zinc-600">{gamesPlayed.toLocaleString()} games played</p>
                )}
              </div>
            </div>
          </Link>
        ...
        <footer className="mt-16 text-center">
          <p className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 dark:text-zinc-700">
